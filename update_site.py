...
for rel in html_files:
    ...
    if '<header class="site-header">' in text:
        continue
    text = re.sub(r'<body>\s*<ul>.*?</ul>', '<body>\n\n    <header class="site-header">\n        <nav class="site-nav" id="site-nav"></nav>\n    </header>', text, count=1, flags=re.S)
    text = text.replace('</body>', '    <script src="nav.js"></script>\n</body>' if not rel.startswith('research/') else '    <script src="../nav.js"></script>\n</body>', 1)
